  ...
  for (const vertex in neighbors) {
    if (!graph.vertices[vertex].fix) {
      vertices[vertex] = [
        ...
        graph.vertices[neighbors[vertex]].level,
        neighbors[vertex],
      ];
...
export async function changeVoterVertex(time = getState().time) {
  ...
  switch (state.protocol) {
    ...
    case "majority":
      ...
      for (const vertex in neighbors) {
        highlightVertices(
          graph.vertices.filter((node) => neighbors[vertex].includes(node.name))
        );
      }
      await sleep(time);
...
function changeOpinion(neighborsArray, random) {
  ...
  for (const vertex in neighborsArray) {
    const neighbors = graph.vertices.filter((node) =>
      neighborsArray[vertex].includes(node.name)
    );
